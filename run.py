...
def reshape_result(result_table):
    """把结果表转换为 96 孔板矩阵形式"""
    df = result_table.loc[:, ["well", "Cq (mean eff) - no plateau - stat efficiency"]]
    df_plate = (
...
def extract_run(input_file):
    ...
    logging.info(f'No experiment given (use option -e). Using "{cli_expList[0]["id"]}"')

    cli_runList = cli_exp.runs()
    if len(cli_runList) < 1:
        logging.error("No runs found!")
        sys.exit(0)
    logging.info(f'No run given (use option -r). Using "{cli_runList[0]["id"]}"')
...
def convert_file(input_file, rdml_file, excel_file):
    """命令行转换：输入 LC96P → RDML + Excel"""
    with tempfile.NamedTemporaryFile(mode="w+", suffix=".tsv", delete=True) as tsv_temp:
